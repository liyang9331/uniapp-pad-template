	...
	export default {
		...
		computed:{
			placeholderStyle(){
				...
				// #ifdef APP-PLUS
					plh = Number((this.height).replace('px','')) + this.$u.sys().statusBarHeight
				// #endif
					
				// #ifdef H5
					plh = this.$u.addUnit(this.$u.getPx(this.height)) + this.$u.sys().statusBarHeight
				// #endif
