	...
	<view class="container-list-item" @click="clickItem($event,item)">
		<label class="content">
			...
			<view class="checkbox" v-else-if ="isRadio" @click.stop="clickBox($event, item)">
				...
				<!-- <i :style="{'color' :checkActiveColor}"  class="txt iconfont icon-add1"/> -->
			</view>
			<!-- 单选框 -->
			<!-- 自定义插槽body -->
			...
			<!-- 自定义插槽body -->
			<view v-if="hasChildren" class="right"><i  class="iconfont icon-z043" :style="{'color':'#000'}"></i></view>
		</label>
	</view>
</template>
...
<style lang="scss" scoped>
	@import  'selected-tree-item.scss';
