<template>
	<view>
		...
		<view class="header">
			<!-- 搜索栏 -->
			<selected-tree-search v-if="searchIf" :backgroundColor="searchBackgroundColor"
				...
				:clearable="searchClearable" @confirm="confirmSearch"></selected-tree-search>
			<!-- 面包屑导航 -->
			<selected-tree-navigation :slabel="props.label" @inF="navigationInt" @clickItem="backTree">
			</selected-tree-navigation>
			<!-- 面包屑导航 -->
		</view>
		<!-- 列表 -->
		<view>
			<view class="container-list" :style="{'padding-bottom' : isCheck ? '130rpx' : 0 , 'padding-top' :searchIf ? '200rpx' :'90rpx' }">
				<selected-tree-item v-for="(item , index) in tree" :key="index" :item="item" :isCheck="isCheck"
...
<script>
	/**
	 * selected-tree 无限树形结构树、支持搜索选择。
	 * @description  无限树形结构组件。支持搜索、选择(包括单选、多选)。面包屑类型导航。原插件地址：https://ext.dcloud.net.cn/plugin?id=2423。
	 * @tutorial url https://ext.dcloud.net.cn/plugin?name=selected-tree
	...
	export default {
		...
		methods: {
			...
			confirmSearch(e) {
				...
				// this.search(this.allData, val);//数据内搜索
				this.$emit("searchKey" , val);//数据重新加载
				// 返回搜索结果
				uni.hideLoading();
				this.setIsre(false); // 设置导航条为搜索状态（数据内搜索设置为true）
