		...
		<view class="content" v-if="!isAdd && !isDetail">
			<view class="query">
				<u--form labelPosition="left" :model="formData" ref="uForm">
					<u-form-item label="出库单编号：" prop="name" class="margin_top_0">
						<u--input v-model="formData.name" style="width: 148rpx;height: 27rpx;" border="none"
							placeholder=""></u--input>
					</u-form-item>
					<u-form-item label="出库单名称：" prop="name" class="margin_top_0">
						<u--input v-model="formData.name" style="width: 148rpx;height: 27rpx;" border="none"
							placeholder=""></u--input>
					</u-form-item>
					...
					<u-form-item label="出库类型：" prop="sex">
						<NextDataSelect label="" style="width: 152rpx;height: 27rpx;" @change="onchange"
							format="{lable}" :filterable="true" :options="options" v-model="formData.name">
						</NextDataSelect>
					</u-form-item>
					...
					<u-form-item label="申领人：" prop="name" class="margin_top_0">
						<u--input v-model="formData.name" style="width: 148rpx;height: 27rpx;" border="none"
							placeholder=""></u--input>
					</u-form-item>
					<u-form-item label="申领单位：" prop="name" class="margin_top_0">
						<u--input v-model="formData.name" style="width: 148rpx;height: 27rpx;" border="none"
							placeholder=""></u--input>
					</u-form-item>
					<u-form-item label="出库状态：" prop="sex">
						<NextDataSelect label="" style="width: 152rpx;height: 27rpx;" @change="onchange"
							format="{lable}" :filterable="true" :options="options" v-model="formData.name">
						</NextDataSelect>
					</u-form-item>
					...
					<zb-table :show-header="true" :columns="column" :stripe="true" :fit="true" show-summary sum-text=""
						...
						<template #status="{data}">
							<view class="table-state">
								<view class="dot"
									:style="{backgroundColor:data.status == 0?'#E50A0A':'#1EBE00'}">
								</view>
								<text
									:style="{color:data.status == 0?'#E50A0A':'#1EBE00'}">{{data.status == 0 ?'未出库':'已出库'}}</text>
							</view>
						</template>
					</zb-table>
